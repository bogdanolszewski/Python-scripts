...
df3 = df3.rename(columns = {"Date ADJ": "Date"})
print(df3)
...
df3 = df3[df3['Date']==(date.today()-timedelta(days=1)).strftime('%Y-%m-%d')]
df3['Shipping'] = df3['Shipping'].astype(np.int64)
df4 = df4[df4['Date']==(date.today()-timedelta(days=1)).strftime('%Y-%m-%d')]
#print(df3.head(15),"\n"); print(df4.head(15),"\n")
df5 = df4.merge(df3, on="Plant", how = "left")
df5 = df5.merge(df3, on="Plant", how = "outer")
df5 = df5.drop(columns=["Date_y", "Shipping_x","Date"])
df5["Date_x"] = df5["Date_x"].fillna(td-timedelta(days=1))
df5["Achieved"] = df5["Achieved"].fillna(0)
df5["Shipping_y"] = df5["Shipping_y"].fillna(0)
df5 = df5.rename(columns={"Date_x": "Date", "Shipping_y": "Shipping"})
df5["Date"] = pd.to_datetime(df5["Date"])

df5['Shipping'] = df5['Shipping'].astype(np.int64)
df5['Achieved'] = df5['Achieved'].astype(np.int64)
df5['weekday'] = (df5['Date'].dt.dayofweek)+1 #+1 because week starts from 0(monday)
df5['weekday'] = df5['weekday'].astype(np.int64)
df5 = df5.iloc[:, [0,1,3,2,4]]
print(df5)
